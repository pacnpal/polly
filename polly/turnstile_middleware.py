...
class TurnstileSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        self.site_key = config('TURNSTILE_SITE_KEY', default='1x00000000000000000000AA')  # Test key
        self.secret_key = config('TURNSTILE_SECRET_KEY', default='1x0000000000000000000000000000000AA')  # Test key
    ...
    async def verify_turnstile_token(self, token: str, client_ip: str) -> bool:
        ...
        if self.secret_key == '1x0000000000000000000000000000000AA':
            logger.info(f"Using Turnstile test key - auto-passing verification for {client_ip}")
            return True
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if success:
                        logger.info(f"Turnstile verification successful for {client_ip}")
                    else:
                        error_codes = result.get('error-codes', [])
                        logger.warning(f"Turnstile verification failed for {client_ip}: {error_codes}")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if turnstile_token:
                ...
                if is_valid:
                    logger.debug(f"Valid Turnstile token from {client_ip} for {request.url.path}")
                    return await call_next(request)
                else:
                    logger.warning(f"Invalid Turnstile token from {client_ip} for {request.url.path}")
                    return JSONResponse(
                        status_code=403,
                ...
                logger.warning(f"No Turnstile token provided from {client_ip} for protected endpoint {request.url.path} - BLOCKING")
                return JSONResponse(
                    status_code=403,
