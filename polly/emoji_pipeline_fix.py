...
class UnifiedEmojiProcessor:
    ...
    async def process_poll_emojis_unified(self, emoji_inputs: List[str], server_id: int, operation: str = "create") -> Tuple[bool, List[str], str]:
        ...
        """
        try:
            logger.info(f"🔄 UNIFIED EMOJI PROCESSOR - Starting {operation} operation with {len(emoji_inputs)} emoji inputs for server {server_id}")
            print(f"🔄 UNIFIED EMOJI PROCESSOR - Starting {operation} operation with {len(emoji_inputs)} emoji inputs for server {server_id}")
            
            # Step 1: Process each emoji input using the Discord handler
            processed_emojis = await self.emoji_handler.process_poll_emojis(emoji_inputs, server_id)
            ...
            try:
                ...
                logger.error(f"❌ UNIFIED EMOJI PROCESSOR - Validation failed: {validation_error}")
                print(f"❌ UNIFIED EMOJI PROCESSOR - Validation failed: {validation_error}")
            ...
            if len(set(validated_emojis)) != len(validated_emojis):
                duplicate_emojis = [emoji for emoji in validated_emojis if validated_emojis.count(emoji) > 1]
            ...
            logger.info(f"🎉 UNIFIED EMOJI PROCESSOR - Successfully processed {len(validated_emojis)} emojis for {operation}: {validated_emojis}")
            print(f"🎉 UNIFIED EMOJI PROCESSOR - Successfully processed {len(validated_emojis)} emojis for {operation}: {validated_emojis}")
    ...
    def extract_emoji_inputs_from_form(self, form_data, num_options: int) -> List[str]:
        ...
        logger.info(f"🔍 EMOJI EXTRACTION - Extracting emojis for {num_options} options")
        ...
        for i in range(1, num_options + 1):
            ...
            logger.info(f"🔍 EMOJI EXTRACTION - {emoji_key}: '{emoji_str}' (len: {len(emoji_str)})")
            print(f"🔍 EMOJI EXTRACTION - {emoji_key}: '{emoji_str}' (len: {len(emoji_str)})")
