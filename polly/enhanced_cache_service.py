...
class EnhancedCacheService(CacheService):
    ...
    async def cache_guild_emojis_extended(self, guild_id: str, emojis: List[Dict[str, Any]]) -> bool:
        ...
        if success:
            logger.info(f"Cached {len(emojis)} emojis for guild {guild_id} with {self.guild_emojis_ttl}s TTL")
        
        return success
    
    async def get_cached_guild_emojis_extended(self, guild_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        if cached_emojis:
            logger.debug(f"Retrieved {len(cached_emojis)} cached emojis for guild {guild_id}")
        
        return cached_emojis
    
    # Live Poll Results Caching
    async def cache_live_poll_results(self, poll_id: int, results_data: Dict[str, Any]) -> bool:
        ...
        return await redis_client.cache_set(cache_key, results_data, self.live_results_ttl)
    
    async def get_cached_live_poll_results(self, poll_id: int) -> Optional[Dict[str, Any]]:
    ...
    async def invalidate_poll_related_cache(self, poll_id: int) -> int:
        ...
        logger.info(f"Invalidated {count} poll-related cache entries for poll {poll_id}")
        return count
            ...
            for name, pattern in patterns.items():
                try:
                    ...
                    if redis_client._client:
                        async for key in redis_client._client.scan_iter(match=f"cache:{pattern}"):
                            keys.append(key)
