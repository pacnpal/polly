...
class BotOwnerLogHandler(logging.Handler):
    ...
    def __init__(self, level=logging.WARNING):
        ...
        self._notification_queue = (
            asyncio.Queue() if hasattr(asyncio, "Queue") else None
        )
...
class BotOwnerNotifier:
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.error("Bot owner has DMs disabled or blocked the bot")
                    return False
                except discord.HTTPException as e:
                    if attempt == 2:
                        logger.error(f"Failed to send DM after 3 attempts: {e}")
                        return False
    ...
    @staticmethod
    async def send_system_status_dm(
        bot: commands.Bot, status: str, details: Optional[Dict[str, Any]] = None
    ):
        ...
        try:
            ...
            color = (
                0x00FF00
                if "healthy" in status.lower() or "recovered" in status.lower()
                else 0xFFAA00
            )
...
class PollError(Exception):
    ...
    def __init__(
        self, message: str, poll_id: Optional[int] = None, recoverable: bool = True
    ):
...
class ErrorRecovery:
    ...
    async def retry_with_backoff(
        func: Callable, *args, max_retries: Optional[int] = None, **kwargs
    ) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                delay = ErrorRecovery.RETRY_DELAYS[
                    min(attempt, len(ErrorRecovery.RETRY_DELAYS) - 1)
                ]
                logger.warning(
...
class PollErrorHandler:
    ...
    @staticmethod
    async def handle_poll_creation_error(
        e: Exception, poll_data: Dict[str, Any], bot: Optional[commands.Bot] = None
    ) -> str:
        ...
        if isinstance(e, ValidationError):
            logger.warning(f"Validation error creating poll '{poll_name}': {e.message}")
            return f"❌ {e.message}"

        # Critical errors that need bot owner notification
        if isinstance(e, discord.Forbidden):
            logger.error(f"Discord permission error creating poll '{poll_name}': {e}")
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Creation - Permission Error", context
                )
        ...
        if isinstance(e, discord.HTTPException):
            ...
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Creation - Discord API Error", context
                )
        ...
        if isinstance(e, DatabaseError):
            ...
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Creation - Database Error", context
                )
        ...
        if bot:
            await BotOwnerNotifier.send_error_dm(
                bot, e, "Poll Creation - Unexpected Error", context
            )
        return "❌ An unexpected error occurred. Please try again."

    @staticmethod
    async def handle_vote_error(
        e: Exception, poll_id: int, user_id: str, bot: Optional[commands.Bot] = None
    ) -> str:
        """Handle voting errors with user-friendly messages and bot owner notifications"""
        context = {"poll_id": poll_id, "user_id": user_id}
        ...
        if isinstance(e, DatabaseError):
            ...
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Voting - Database Error", context
                )
        ...
        if bot:
            await BotOwnerNotifier.send_error_dm(
                bot, e, "Voting - Unexpected Error", context
            )
        return "❌ An unexpected error occurred while voting. Please try again."

    @staticmethod
    async def handle_poll_closure_error(
        e: Exception, poll_id: int, bot: Optional[commands.Bot] = None
    ) -> str:
        """Handle poll closure errors with user-friendly messages and bot owner notifications"""
        context = {"poll_id": poll_id}

        if isinstance(e, ValidationError):
            logger.warning(f"Validation error closing poll {poll_id}: {e.message}")
            return f"❌ {e.message}"
        ...
        if isinstance(e, DatabaseError):
            ...
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Closure - Database Error", context
                )
            return "❌ Database error closing poll. Please try again."

        if isinstance(e, discord.Forbidden):
            logger.error(f"Discord permission error closing poll {poll_id}: {e}")
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Closure - Permission Error", context
                )
        ...
        if isinstance(e, discord.HTTPException):
            ...
            if bot:
                await BotOwnerNotifier.send_error_dm(
                    bot, e, "Poll Closure - Discord API Error", context
                )
        ...
        if bot:
            await BotOwnerNotifier.send_error_dm(
                bot, e, "Poll Closure - Unexpected Error", context
            )
        return "❌ An unexpected error occurred while closing poll. Please try again."

    @staticmethod
    async def handle_scheduler_error(
        e: Exception, poll_id: int, operation: str, bot: Optional[commands.Bot] = None
    ) -> bool:
        """Handle scheduler errors and attempt recovery with bot owner notifications"""
        context = {"poll_id": poll_id, "operation": operation}

        logger.error(f"Scheduler error for poll {poll_id} during {operation}: {e}")
        logger.exception("Full traceback for scheduler error:")

        # Always notify bot owner of scheduler errors as they're critical
        if bot:
            await BotOwnerNotifier.send_error_dm(
                bot, e, f"Scheduler Error - {operation}", context
            )
                ...
                try:
                    ...
                    if poll:
                        ...
                        if (
                            ...
                            logger.info(f"Rescheduled closing for poll {poll_id}")
...
class DiscordErrorHandler:
    ...
    async def safe_send_message(
        channel, content=None, embed=None, max_retries: int = 3
    ...
    @staticmethod
    async def safe_add_reactions(
        message: discord.Message, emojis: List[str], bot: Optional[commands.Bot] = None
    ) -> List[str]:
        ...
        for emoji in emojis:
            try:
                ...
                prepared_emoji = (
                    emoji_handler.prepare_emoji_for_reaction(emoji)
                    if emoji_handler
                    else emoji
                )
                ...
                logger.debug(
                    f"Successfully added reaction {prepared_emoji} (original: {emoji})"
                )
            except discord.Forbidden:
    ...
    async def safe_edit_message(
        message: discord.Message, content=None, embed=None, max_retries: int = 3
    ) -> bool:
...
class DatabaseHealthChecker:
    ...
    def check_poll_integrity(db, poll_id: int) -> Dict[str, Any]:
        ...
        if not str(getattr(poll, "server_id", "")) or not str(
            getattr(poll, "channel_id", "")
        ):
...
async def handle_critical_error(
    ...
    if bot:
        await BotOwnerNotifier.send_error_dm(
            bot, error, f"Critical System Error - {operation}", context
        )
...
async def notify_bot_owner_of_error(
    error: Exception,
    operation: str,
    context: Optional[Dict[str, Any]] = None,
    bot: Optional[commands.Bot] = None,
    ...
    try:
        ...
        if not bot:
            ...
            try:
                ...
                logger.warning("Could not import bot instance for error notification")
                return False
...
def notify_bot_owner_of_error_sync(
    ...
    try:
        ...
        try:
            ...
            loop.create_task(notify_bot_owner_of_error(error, operation, context))
        except RuntimeError:
        ...
        logger.error(f"Failed to send sync bot owner notification for {operation}: {e}")
        return False
