...
from .database import get_db_session, Guild, Channel, Poll, Vote, POLL_EMOJIS
...
async def update_channels_cache(bot: commands.Bot, guild: discord.Guild):
    ...
    try:
        ...
        db.commit()
        logger.info(f"Updated channel cache for {guild.name} ({len(guild.channels)} channels)")
...
async def create_poll_embed(poll: Poll, show_results: bool = True) -> discord.Embed:
    ...
    if show_results and poll.should_show_results():
        ...
        if poll.status == "closed" and total_votes > 0:
            ...
            if winners:
                winner_text = ", ".join([f"{(poll.emojis[i] if i < len(poll.emojis) else POLL_EMOJIS[i])} {poll.options[i]}" for i in winners])
                embed.add_field(name="🏆 Winner(s)", value=winner_text, inline=True)
    else:
...
async def post_poll_to_channel(bot: commands.Bot, poll: Poll) -> bool:
    ...
    try:
        ...
        if not channel:
            logger.error(f"Channel {poll.channel_id} not found for poll {poll.id}")
            return False
