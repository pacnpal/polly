    ...
    async def find_emoji_by_name(self, guild_id: int, emoji_name: str) -> Optional[discord.Emoji]:
        ...
        try:
            ...
            if emoji:
                ...
                logger.debug(f"Custom emoji '{clean_name}' not found in guild {guild.name}")
    ...
    async def process_emoji_input(self, emoji_input: str, guild_id: int) -> Optional[str]:
        ...
        if custom_emoji_data:
            ...
            try:
                ...
                if emoji:
                    logger.debug(f"Valid custom emoji: {emoji.name} ({emoji.id})")
                    return str(emoji)  # Returns <:name:id> format
                else:
                    logger.warning(f"Custom emoji {custom_emoji_data['id']} not accessible")
            except Exception as e:
        ...
        for i, emoji_input in enumerate(emoji_inputs):
            if not emoji_input or not emoji_input.strip():
                ...
                if i < len(default_emojis):
                    ...
                    logger.debug(f"Using default emoji for option {i}: {default_emojis[i]}")
                continue
            ...
            if processed_emoji:
                ...
                logger.debug(f"Processed emoji for option {i}: {processed_emoji}")
            else:
                # Fallback to default emoji
                if i < len(default_emojis):
                    processed_emojis.append(default_emojis[i])
                    logger.warning(f"Failed to process emoji '{emoji_input}', using fallback: {default_emojis[i]}")
