"""
Polly Poll Operations
Core poll business logic with bulletproof operations and comprehensive error handling.
"""
...
from .error_handler import PollErrorHandler, DiscordErrorHandler, critical_operation, notify_error_async
...
class BulletproofPollOperations:
            ...
            try:
                ...
                try:
                    ...
                    if TypeSafeColumn.get_string(poll, 'status') != "active":
                        db.rollback()
                        return {"success": False, "error": f"Poll is not active (status: {TypeSafeColumn.get_string(poll, 'status')})"}
                    ...
                    multiple_choice_value = TypeSafeColumn.get_bool(
                        poll, 'multiple_choice', False)
                    if multiple_choice_value:
                    ...
                    if vote_action == "removed":
                        ...
                        multiple_choice_verification = TypeSafeColumn.get_bool(
                            poll, 'multiple_choice', False)
                        if multiple_choice_verification is True:
    ...
    async def bulletproof_poll_closure(self, poll_id: int, reason: str = "manual") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if TypeSafeColumn.get_string(poll, 'status') == "closed":
    ...
    def _generate_poll_results(self, poll: Poll) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "poll_id": TypeSafeColumn.get_int(poll, 'id'),
                "title": TypeSafeColumn.get_string(poll, 'name'),
            ...
            notify_error(e, "Poll Results Generation", poll_id=poll_id)
            return {
                "poll_id": poll_id,
...
# Utility functions for image and file management
async def cleanup_image(image_path: str) -> bool:
    ...
    try:
        ...
        await notify_error_async(e, "Image Cleanup", image_path=image_path)
    return False
...
async def cleanup_poll_images(poll_id: int) -> None:
    ...
    try:
        ...
        await notify_error_async(e, "Poll Image Cleanup", poll_id=poll_id)
    finally:
        db.close()
