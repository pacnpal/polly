...
async def get_security_status(request: Request, current_user: DiscordUser = Depends(require_auth)) -> JSONResponse:
    ...
    for ip in blocked_ips:
        ip_details.append({
            "ip": ip,
            "violation_count": ip_blocker.get_violation_count(ip)
        })
...
async def unblock_ip(request: Request, current_user: DiscordUser = Depends(require_auth)) -> JSONResponse:
...
async def get_system_health(request: Request, current_user: DiscordUser = Depends(require_auth)) -> JSONResponse:
...
def add_admin_routes(app):
    ...
    async def admin_security_status(request: Request, current_user: DiscordUser = Depends(require_auth)):
        return await get_security_status(request, current_user)
    
    @app.post("/admin/security/unblock")
    async def admin_unblock_ip(request: Request, current_user: DiscordUser = Depends(require_auth)):
        return await unblock_ip(request, current_user)
    
    @app.get("/admin/health")
    async def admin_system_health(request: Request, current_user: DiscordUser = Depends(require_auth)):
