...
class TestImageGenerator:
    ...
    def __init__(self, use_real_images: bool = False):
        ...
        self.color_schemes = {
            ...
            "pastel": [(255, 182, 193), (173, 216, 230), (144, 238, 144), (255, 218, 185)],
            "dark": [(64, 64, 64), (128, 0, 0), (0, 128, 0), (0, 0, 128)],
            "monochrome": [(255, 255, 255), (192, 192, 192), (128, 128, 128), (64, 64, 64)],
    ...
    def _setup_real_images(self) -> None:
        ...
        try:
            ...
            if os.path.exists(self.sample_images_path):
                ...
                logger.warning(f"Sample images directory not found: {self.sample_images_path}")
    ...
    def create_text_image(self, text: str, size: Tuple[int, int],
    ...
    def create_solid_color_image(self, size: Tuple[int, int], 
    ...
    def create_gradient_image(self, size: Tuple[int, int], 
    ...
    def create_pattern_image(self, size: Tuple[int, int], 
        ...
        try:
            ...
            if pattern_type == "checkerboard":
                ...
                for y in range(0, size[1], square_size):
                    for x in range(0, size[0], square_size):
                        color_index = ((x // square_size) + (y // square_size)) % len(colors)
                ...
                for x in range(0, size[0], stripe_width):
                    ...
                    draw.rectangle([x, 0, x + stripe_width, size[1]], 
                                 fill=colors[color_index])
                ...
                for y in range(dot_radius, size[1], dot_spacing):
                    for x in range(dot_radius, size[0], dot_spacing):
                        color_index = ((x // dot_spacing) + (y // dot_spacing)) % len(colors)
            ...
            return self.create_solid_color_image(size, colors[0] if colors else (128, 128, 128), format)
    ...
    def get_test_image_set(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, config in self.image_configs.items():
            ...
            test_images[f"{name}_text"] = {
                "data": self.create_text_image(f"Test {name.title()}", size, format=format),
        ...
        special_cases = [
            {
                "name": "very_large",
                "data": self.create_text_image("Large Test", (2048, 1536), format="JPEG"),
            ...
            {
                "name": "gradient",
                "data": self.create_gradient_image((600, 400), (255, 0, 0), (0, 0, 255)),
            ...
            {
                "name": "stripes",
                "data": self.create_pattern_image((500, 300), "stripes", 
    ...
    def get_image_for_scenario(self, scenario_type: str, poll_id: Any = None) -> Tuple[bytes, str]:
        ...
        if scenario_type == "small":
            return (self.create_text_image(f"Poll {poll_id_str}", (200, 150)), 
                   f"poll_{poll_id_str}_small.png")
        
        elif scenario_type == "medium":
            return (self.create_text_image(f"Poll {poll_id_str}", (600, 400)), 
                   f"poll_{poll_id_str}_medium.png")
        
        elif scenario_type == "large":
            return (self.create_gradient_image((1200, 800), (100, 200, 255), (255, 100, 200)), 
                   f"poll_{poll_id_str}_large.png")
        
        elif scenario_type == "pattern":
            return (self.create_pattern_image((400, 300), "checkerboard"), 
                   f"poll_{poll_id_str}_pattern.png")
        
        elif scenario_type == "unicode":
            return (self.create_text_image(f"Poll {poll_id_str} 🎉 测试", (500, 200)), 
                   f"poll_{poll_id_str}_unicode.png")
        
        elif scenario_type == "minimal":
            return (self.create_minimal_png(), 
                   f"poll_{poll_id_str}_minimal.png")
        
        else:  # default
            return (self.create_text_image(f"Test Poll {poll_id_str}", (400, 300)), 
...
def get_test_image_for_poll(poll_id: Any = None, image_type: str = "default", use_real_images: bool = False) -> Tuple[bytes, str]:
