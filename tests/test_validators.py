...
class TestPollValidator:
    ...
    def test_validate_poll_options_sanitization(self):
        ...
        assert any(
            "quotes" in option and "'" not in option and '"' not in option
            for option in result
        )
        assert any(
            "script" in option and "<" not in option and ">" not in option
            for option in result
        )
    ...
    def test_validate_server_and_channel_valid(self):
        ...
        for server_id, channel_id in valid_pairs:
            result_server, result_channel = PollValidator.validate_server_and_channel(
    ...
    def test_validate_timezone_valid(self, timezone_test_cases):
        ...
        for tz in timezone_test_cases:
            if tz in [
                ...
                assert result in pytz.all_timezones or result in [
                    "US/Eastern",
                    "US/Central",
                    "US/Mountain",
                    "US/Pacific",
    ...
    def test_validate_image_file_valid(self, mock_file_upload):
        ...
        is_valid, error = PollValidator.validate_image_file(
            mock_file_upload, b"fake_data"
        )
    ...
    def test_validate_poll_data_malicious(self, malicious_inputs):
        ...
        for input_name, malicious_value in malicious_inputs.items():
            malicious_data = {
                ...
                "question": malicious_value
                if input_name == "question"
                else "Test question?",
                "options": [malicious_value, "Option 2"]
                if input_name == "options"
                else ["Option 1", "Option 2"],
                "server_id": malicious_value
                if input_name == "server_id"
                else "123456789",
                "channel_id": malicious_value
                if input_name == "channel_id"
                else "987654321",
...
class TestVoteValidator:
    ...
    def test_validate_existing_vote(
        self, mock_get_db_session, sample_poll, sample_user
    ):
