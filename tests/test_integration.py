...
class TestPollCreationWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_poll_creation_workflow(
        self, db_session, mock_bot, mock_scheduler, sample_discord_user
    ):
        ...
        with (
            patch(
                "polly.validators.PollValidator.validate_poll_data",
                return_value=poll_data,
            ),
            ...
            result = await create_poll_htmx(
                mock_request, mock_bot, mock_scheduler, sample_discord_user
            )
    ...
    @pytest.mark.asyncio
    async def test_voting_workflow(
        self, db_session, mock_bot, sample_poll, sample_user
    ):
        ...
        with (
            ...
            mock_ops = Mock()
            mock_ops.bulletproof_vote_collection = AsyncMock(
                return_value={
                    "success": True,
                    "action": "added",
...
class TestDataFlowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_poll_data_consistency(
        self, db_session, mock_bot, sample_discord_user
    ):
...
class TestErrorHandlingIntegration:
    ...
    async def test_database_error_recovery(self, mock_bot, sample_discord_user):
        ...
        with (
            patch(
                "polly.htmx_endpoints.get_db_session",
    ...
    async def test_validation_error_propagation(
        ...
        invalid_poll_data = {
            ...
            - timedelta(hours=1),  # Invalid: in past
        ...
        with patch("polly.htmx_endpoints.get_db_session", return_value=db_session):
            ...
            result = await create_poll_htmx(
                mock_request, mock_bot, Mock(), sample_discord_user
            )
...
class TestPerformanceIntegration:
    ...
    async def test_large_poll_handling(self, db_session, mock_bot, sample_discord_user):
        ...
        with (
            ...
            try:
                result = await get_poll_dashboard_htmx(
                    large_poll.id, mock_request, mock_bot, sample_discord_user
                )
...
class TestSecurityIntegration:
    ...
    async def test_authorization_flow(self, db_session, sample_discord_user):
        ...
        with patch("polly.htmx_endpoints.get_db_session", return_value=db_session):
            ...
            try:
                result = await get_poll_edit_form(
                    other_user_poll.id, mock_request, Mock(), sample_discord_user
                )
    ...
    @pytest.mark.asyncio
    async def test_input_sanitization_flow(
        self, db_session, mock_bot, sample_discord_user, malicious_inputs
    ):
        ...
        for input_name, malicious_value in malicious_inputs.items():
            try:
                ...
                with patch(
                    "polly.htmx_endpoints.get_db_session", return_value=db_session
                ):
                    from polly.htmx_endpoints import create_poll_htmx
                    from fastapi import Request
