...
from polly.background_tasks import (
    ...
    open_poll, close_poll, restore_scheduled_jobs,
    start_reaction_safeguard, reaction_safeguard_task
)
...
class TestPollOperations:
    ...
    async def test_open_poll_discord_error(self, sample_poll, db_session, mock_bot):
        ...
        with patch('polly.background_tasks.get_db_session', return_value=db_session), \
             patch('polly.background_tasks.get_bot_instance', return_value=mock_bot), \
             patch('polly.background_tasks.post_poll_to_discord', side_effect=Exception("Discord error")) as mock_post, \
             patch('polly.background_tasks.notify_error_async') as mock_notify:
    ...
    async def test_close_poll_discord_error(self, sample_poll, db_session, mock_bot):
        ...
        with patch('polly.background_tasks.get_db_session', return_value=db_session), \
             patch('polly.background_tasks.get_bot_instance', return_value=mock_bot), \
             patch('polly.background_tasks.update_poll_message', side_effect=Exception("Discord error")) as mock_update, \
             patch('polly.background_tasks.notify_error_async') as mock_notify:
...
class TestJobRestoration:
    ...
    @pytest.mark.asyncio
    async def test_restore_scheduled_jobs_active_polls(self, db_session, mock_scheduler):
    ...
    async def test_restore_scheduled_jobs_database_error(self, mock_scheduler):
        ...
        with patch('polly.background_tasks.get_db_session', side_effect=Exception("DB error")), \
             patch('polly.background_tasks.notify_error_async') as mock_notify:
    ...
    @pytest.mark.asyncio
    async def test_restore_scheduled_jobs_timezone_error(self, db_session, mock_scheduler):
...
class TestReactionSafeguard:
    ...
    async def test_reaction_safeguard_task_success(self, db_session, mock_bot):
        ...
        with patch('polly.background_tasks.get_db_session', return_value=db_session), \
             patch('polly.background_tasks.get_bot_instance', return_value=mock_bot), \
             patch('asyncio.sleep', side_effect=Exception("Stop loop")):  # Stop the infinite loop
...
class TestBackgroundTasksEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_scheduler_with_extreme_dates(self, db_session, mock_scheduler, datetime_edge_cases):
...
class TestBackgroundTasksIntegration:
    ...
    async def test_scheduler_error_recovery(self, mock_scheduler):
        ...
        with patch('polly.background_tasks.get_scheduler', return_value=mock_scheduler), \
             patch('polly.background_tasks.notify_error_async') as mock_notify:
