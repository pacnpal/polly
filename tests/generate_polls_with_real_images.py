...
class RealImagePollGenerator:
    ...
    async def create_poll_with_real_image(
        self, config: Dict[str, Any], loop_num: int, poll_num: int
    ) -> bool:
        ...
        try:
            ...
            title = (
                f"Real Image Poll {loop_num}-{poll_num} ({config['options']} options)"
            )
            ...
            if result["success"]:
                ...
                logger.info(
                    f"✅ Created poll {poll_id}: {title} with image {image_filename}"
                )
                return True
            else:
    ...
    async def create_poll_with_specific_image(
        self, image_path: str, image_index: int
    ) -> bool:
        ...
        try:
            ...
            if result["success"]:
                ...
                error_msg = result.get("error", "Unknown error")
                logger.error(
                    f"❌ Failed to create poll for {image_filename}: {error_msg}"
                )
                self.errors += 1
                return False
            ...
            image_filename = (
                os.path.basename(image_path) if "image_path" in locals() else "unknown"
            )
    ...
    async def generate_polls_for_all_images(self) -> Dict[str, Any]:
        ...
        logger.info("All images poll generation completed:")
    ...
    async def generate_all_polls(self) -> Dict[str, Any]:
        ...
        if not self.image_generator.real_images_cache:
            logger.error(
                "No real images available. Make sure the sample-images repository is accessible."
            )
            return {
                "success": False,
        ...
        logger.info("Real image poll generation completed:")
        logger.info(f"  - Polls created: {result['polls_created']}")
        logger.info(f"  - Errors: {result['errors']}")
        logger.info(f"  - Duration: {result['duration']:.2f} seconds")
...
async def main():
    ...
    if args.dry_run:
        ...
        generator = RealImagePollGenerator(
            loops=args.loops, cleanup=not args.no_cleanup
        )
        ...
        if args.use_all_images:
            ...
            logger.info(
                f"Poll configurations will rotate through {len(generator.poll_configs)} different configs"
            )
        else:
            ...
            for i, config in enumerate(generator.poll_configs, 1):
                logger.info(
                    f"  Config {i}: {config['options']} options, "
    ...
    try:
        ...
        if args.use_all_images:
            ...
            logger.info(
                f"Creating polls for all {len(generator.image_generator.real_images_cache)} images"
            )
            ...
            if result["success"]:
                ...
                print(
                    f"📊 Created {result['polls_created']} polls in {result['duration']:.2f} seconds"
                )
            ...
            if result["success"]:
                ...
                print(
                    f"🖼️  Used {result['real_images_used']} real images from sample-images repository"
                )
