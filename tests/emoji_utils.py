...
def get_random_poll_emojis(option_count: int = 4) -> List[str]:
    ...
    if option_count <= len(common_poll_emojis):
        ...
        all_emojis = list(emoji.EMOJI_DATA.keys())
        additional_emojis = [
            emoji_char for emoji_char in all_emojis if emoji_char not in selected_emojis
        ]

        if additional_emojis:
            selected_emojis.extend(
                random.sample(
                    additional_emojis, min(remaining_count, len(additional_emojis))
                )
            )
