...
def migrate_database():
    ...
    if not os.path.exists(db_path):
        print(
            f"Database file {db_path} not found. No migration needed for new installations."
        )
        return True
    ...
    try:
        ...
        missing_polls = [
            col for col in required_polls_columns if col not in polls_columns
        ]

        required_prefs_columns = ["last_role_id"]
        missing_prefs = [
            col for col in required_prefs_columns if col not in prefs_columns
        ]
