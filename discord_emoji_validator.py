...
class EmojiValidator:
    ...
    async def get_message_from_url(self, message_url: str) -> discord.Message:
        ...
        try:
            ...
            url_pattern = (
                r"https://(?:discord|discordapp)\.com/channels/(\d+)/(\d+)/(\d+)"
            )
            ...
            logger.info(
                f"Parsed URL - Guild: {guild_id}, Channel: {channel_id}, Message: {message_id}"
            )
            ...
            if not channel:
                logger.info(
                    f"Channel {channel_id} not found in cache, attempting to fetch..."
                )
                # Try fetching the channel if not in cache
                try:
                    channel = await self.bot.fetch_channel(channel_id)
                    logger.info(
                        f"Successfully fetched channel: {channel.name} ({channel_id})"
                    )
            ...
            try:
                logger.info(
                    f"Attempting to fetch message {message_id} from channel {channel_id}"
                )
                message = await channel.fetch_message(message_id)
                logger.info(
                    f"Successfully retrieved message {message_id} from {channel.name}"
                )
                return message
            except discord.NotFound:
    ...
    async def test_emoji_reaction(
        self, emoji_input: str, test_message_url: str = None
    ) -> dict:
        """
        ...
        logger.info(
            f"Test message found: {test_message.id} in {test_message.channel.name}"
        )
        ...
        try:
            ...
            logger.info(
                f"Attempting to add reaction '{emoji_to_use}' to message {test_message.id}"
            )
            ...
            if e.code == 10014:
                logger.error(
                    f"❌ Unknown Emoji error (10014): '{emoji_input}' -> '{emoji_to_use}'"
                )
                result["error"] = (
                    "Unknown Emoji (10014): Emoji not found or invalid format"
                )
            else:
            ...
            logger.error(
                f"❌ Invalid emoji format: '{emoji_input}' -> '{emoji_to_use}'"
            )
    ...
    def _prepare_emoji_for_reaction(self, emoji_input: str) -> str:
        ...
        if self.is_unicode_emoji(emoji_input):
            ...
            logger.debug(
                f"Unicode emoji cleaned (variation selectors removed): '{emoji_input}' -> '{cleaned}'"
            )
            return cleaned
    ...
    def is_unicode_emoji(self, text: str) -> bool:
        ...
        try:
            ...
            for char in cleaned_text:
                if not unicodedata.category(char).startswith(
                    "So"
                ) and not unicodedata.name(char, "").startswith("EMOJI"):
                    ...
                    if unicodedata.category(char) not in [
                        ...
                        logger.debug(
                            f"Character '{char}' (category: {unicodedata.category(char)}) is not emoji-related"
                        )
                        return False
    ...
    def get_emoji_by_id(self, emoji_id: int) -> discord.Emoji:
        ...
        if emoji:
            logger.debug(
                f"Found emoji: {emoji.name} ({emoji_id}) from {emoji.guild.name}"
            )
        else:
    ...
    def get_emoji_by_name(
        self, name: str, guild: discord.Guild = None
    ) -> discord.Emoji:
        """
        ...
        """
        logger.debug(
            f"Looking up emoji by name: '{name}'"
            + (f" in guild {guild.name}" if guild else " globally")
        )
        ...
        if guild:
            ...
            if emoji:
                logger.debug(
                    f"Found emoji globally: {emoji.name} ({emoji.id}) from {emoji.guild.name}"
                )
            else:
    ...
    def is_emoji_usable(
        self, emoji: discord.Emoji, guild: discord.Guild = None
    ) -> bool:
        """
    ...
    async def validate_emoji(
        ...
        test_reaction: bool = False,
        test_message_url: str = None,
    ) -> dict:
        """
        ...
        """
        logger.info(
            f"Starting emoji validation for: '{emoji_input}'"
            + (f" in guild {guild.name}" if guild else " globally")
        )
            ...
            result.update(
                {
                    ...
                    "details": {
                        "name": unicodedata.name(
                            emoji_input[0], "Unknown Unicode Character"
                        )
                    },
                }
            )
        ...
        # Check if it's a custom emoji format
            ...
            if emoji_obj:
                logger.info(
                    f"Custom emoji found in cache: {emoji_obj.name} from {emoji_obj.guild.name}"
                )
            else:
                logger.warning(
                    f"Custom emoji with ID {parsed['id']} not found in bot cache"
                )
            ...
            if emoji_obj:
                logger.info(
                    f"✅ Found emoji by name: {emoji_obj.name} ({emoji_obj.id})"
                )
                result.update(
                    {
                        "valid": True,
                ...
                logger.warning(
                    f"❌ Emoji '{emoji_input}' not found - invalid format or not accessible"
                )
        ...
        if test_reaction and result["valid"] and test_message_url:
            ...
            if result["type"] in ["custom", "custom_by_name"] and result["emoji"]:
                emoji_for_reaction = str(
                    result["emoji"]
                )  # This gives us <:name:id> format
                logger.info(
                    f"Using emoji object format for reaction test: {emoji_for_reaction}"
                )

            reaction_test = await self.test_emoji_reaction(
                emoji_for_reaction, test_message_url
            )
            result["can_react"] = reaction_test["can_react"]
            result["reaction_test"] = reaction_test
        elif test_reaction:
            logger.warning(
                "Reaction testing requested but no test message URL provided or emoji invalid"
            )

        logger.info(
            f"Emoji validation completed. Valid: {result['valid']}, Type: {result['type']}, Usable: {result['usable']}, Can React: {result['can_react']}"
        )
        return result
...
async def on_guild_emojis_update(guild, before, after):
    ...
    logger.info(
        f"🔄 Guild {guild.name} emoji update: {len(before)} -> {len(after)} emojis"
    )
...
async def on_command(ctx):
    ...
    logger.info(
        f"🔧 Command '{ctx.command}' invoked by {ctx.author} in {ctx.guild.name if ctx.guild else 'DM'}"
    )
...
async def validate_emoji_command(ctx, *, emoji_input: str):
    ...
    """
    logger.info(
        f"Validate emoji command called by {ctx.author} in {ctx.guild.name if ctx.guild else 'DM'}: '{emoji_input}'"
    )
    ...
    logger.info("Sending validation result embed to user...")
    ...
    embed.add_field(
        name="Valid", value="✅ Yes" if result["valid"] else "❌ No", inline=True
    )
    embed.add_field(name="Type", value=result["type"] or "Invalid", inline=True)

    if result["valid"]:
        embed.add_field(
            name="Usable", value="✅ Yes" if result["usable"] else "❌ No", inline=True
        )
        ...
        if result["reaction_test"] is not None:
            ...
            if result["reaction_test"]["error"]:
                embed.add_field(
                    name="Reaction Error",
                    value=result["reaction_test"]["error"],
                    inline=False,
                )
                if result["reaction_test"].get("emoji_encoded"):
                    embed.add_field(
                        name="Encoded Format",
                        value=f"`{result['reaction_test']['emoji_encoded']}`",
                        inline=False,
                    )
            else:
                test_details = []
            ...
            embed.add_field(
                name="Reaction Test", value="❌ Failed to run test", inline=True
            )
        else:
            embed.add_field(
                name="Reaction Test",
                value="⚠️ No test message URL configured",
                inline=True,
            )
        ...
        if result["type"] == "unicode":
            embed.add_field(
                name="Character Name",
                value=result["details"].get("name", "Unknown"),
                inline=False,
            )
            ...
            if result["emoji"]:
                ...
                embed.add_field(
                    name="From Guild",
                    value=emoji_obj.guild.name if emoji_obj.guild else "Unknown",
                    inline=True,
                )
                ...
                embed.add_field(
                    name="Note",
                    value="Emoji format is valid but not found in bot's cache",
                    inline=False,
                )
        ...
        embed.add_field(
            name="Reason",
            value=result["details"].get("reason", "Unknown error"),
            inline=False,
        )
...
async def test_reaction_command(ctx, *, emoji_input: str):
    ...
    if "#" in emoji_input:
        ...
        logger.info(
            f"Detected comment in input, using cleaned emoji: '{original_input}' -> '{emoji_input}'"
        )
    else:
    ...
    logger.info(
        f"Test reaction command called by {ctx.author} in {ctx.guild.name if ctx.guild else 'DM'}: '{emoji_input}'"
    )
    ...
    try:
        ...
        await ctx.send(
            "❌ No test message URL configured in .env file!\nAdd: `TEST_MESSAGE_URL=https://discord.com/channels/guild_id/channel_id/message_id`"
        )
        return

    embed = discord.Embed(title="Reaction Test Result", color=discord.Color.blue())
    ...
    if not emoji_validator.is_unicode_emoji(
        emoji_input
    ) and not emoji_validator.is_custom_emoji_format(emoji_input):
        logger.info(
            f"Emoji appears to be a name, attempting to resolve: '{emoji_input}'"
        )
    ...
    if result["can_react"]:
        ...
        embed.add_field(
            name="Result", value="❌ Cannot be used as reaction", inline=True
        )
    ...
    if result.get("emoji_encoded"):
        embed.add_field(
            name="Encoded Format Used",
            value=f"`{result['emoji_encoded']}`",
            inline=False,
        )
    ...
    url_display = (
        test_message_url[:50] + "..."
        if len(test_message_url) > 50
        else test_message_url
    )
...
async def emoji_info_command(ctx, emoji_id: int):
    ...
    embed = discord.Embed(
        title=f"Emoji Information: {emoji.name}", color=discord.Color.blue()
    )
    ...
    embed.add_field(
        name="Guild", value=emoji.guild.name if emoji.guild else "Unknown", inline=True
    )
    embed.add_field(
        name="Usable",
        value="✅ Yes"
        if emoji_validator.is_emoji_usable(emoji, ctx.guild)
        else "❌ No",
        inline=True,
    )
    embed.add_field(
        name="Created At",
        value=emoji.created_at.strftime("%Y-%m-%d %H:%M:%S UTC"),
        inline=True,
    )
...
async def emoji_help_command(ctx):
    ...
    commands_info = [
        (
            ...
            "Validate emoji and test as reaction (if configured)",
        ),
